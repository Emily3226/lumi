...
This is for the case where there's no old Neon/Postgres connection to pull
from (or it's unavailable), but a local SQLite copy with the same schema
already has real data in it.

Usage (run from the repo root, with MONGODB_URI already set in .env):
...
def main() -> None:
    ...
    # Keep id counters ahead of the highest migrated id so future
    # next_id() calls (new bookings/slots created via the admin panel or
    # the app itself) don't collide with migrated ones.
