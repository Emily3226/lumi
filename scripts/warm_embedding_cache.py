...
Pre-downloads and caches the local embedding model (see rag/embeddings.py)
so it's already on disk before the app starts serving traffic, instead of
downloading it lazily the first time a chat message needs mentor matching
or contest retrieval.
...
from rag.embeddings import CACHE_DIR, MODEL_NAME, _load_model
