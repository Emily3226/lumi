...
def build_problem_set_from_text(text: str) -> ProblemSetResult:
    ...
    solutions_url = None
    # Solutions are now generated by default alongside every problem set
    # (previously this was opt-in via _wants_solutions(text) because it
    # roughly doubles render time; if problem-set requests start timing out
    # again, that's the first thing to revisit — e.g. gate on `count`, or
    # render solutions lazily on request instead of eagerly here).
    sol_pdf = None
    try:
        sol_pdf = fitz.open()
        sol_written = 0
    ...
    if solutions_url:
        reply_text = (
            f"I generated a mixed-year {label}, plus a matching solutions PDF. "
            f"Use the download buttons below to open the problems and the solutions."
        )
    else:
        reply_text = (
            f"I generated a mixed-year {label}. Official solutions weren't available "
            f"for these problems, so only the problem set PDF is ready below."
        )
